...
class StoreController extends Controller
{
    ...
    public function actionAnalytics()
    {
        //if (\Yii::$app->request->isPost) {
            ...
            $post = [
                'begin_date'=>1536549458, //9-10
                'end_date'=>1537067858    //9-16
            ];

            $data = [
                'begin_date' =>date('Ymd',$post['begin_date']),
                'end_date' => date('Ymd',$post['end_date']),
            ]; 
        //}

        $access_token = $this->wechat->getAccessToken();
        $api = "https://api.weixin.qq.com/datacube/getweanalysisappidvisitdistribution?access_token={$access_token}";
    ...
    public function actionSetting()
    {
        if (\Yii::$app->request->isPost) {
            ...
            $option = Option::getList([
                ...
                'web_service', 'web_service_url', 'payment', 'wxapp', 'quick_navigation',
    ...
    public function actionShopSend($id = null)
    {
        ...
        $form->load(['id' => $id], '');
        $list = $form->search();
        return $this->render('shop-send.php', $list);
